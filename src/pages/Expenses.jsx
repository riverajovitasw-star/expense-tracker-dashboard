...
import { Plus, Search, Filter, Edit2, Trash2, CreditCard, X } from 'lucide-react';
import { format } from 'date-fns';
import toast from 'react-hot-toast';

const CATEGORIES = ['Food', 'Transportation', 'Shopping', 'Bills', 'Entertainment', 'Education', 'Health', 'Other'];

const emptyForm = { title: '', amount: '', category: 'Food', date: format(new Date(), 'yyyy-MM-dd'), notes: '' };
    ...
    <form onSubmit={submit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          ...
          <input name="title" value={form.title} onChange={handle} placeholder="e.g. Grocery Shopping"
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button type="submit" disabled={loading} className="btn-primary flex-1">
          {loading ? <span className="flex items-center justify-center gap-2"><div className="w-4 h-4 rounded-full border-2 border-white border-t-transparent animate-spin" /> Saving...</span> : (initial ? 'Update Expense' : 'Add Expense')}
        </button>
      </div>
    </form>
  );
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="glass-card p-4 border border-red-500/15">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="font-display font-bold text-red-400 text-2xl mt-1">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="flex-1 min-w-[180px] relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Search expenses..." className="input-field pl-9 py-2.5 text-sm" />
          </div>
      ...
      <div className="glass-card overflow-hidden">
        {loading ? (
          ...
          <EmptyState icon={CreditCard} title="No expenses found"
            ...
            action={<button onClick={() => setShowModal(true)} className="btn-primary text-sm flex items-center gap-2"><Plus size={14} /> Add Expense</button>}
          />
        ) : (
                ...
                {expenses.map((exp, i) => (
                  <tr key={exp._id}
                    ...
                    <td className="px-5 py-3.5">
                      <p className="text-gray-200 text-sm font-medium">{exp.title}</p>
                      {exp.notes && <p className="text-gray-500 text-xs mt-0.5 truncate max-w-[200px]">{exp.notes}</p>}
                    </td>
                    <td className="px-5 py-3.5"><CategoryBadge category={exp.category} /></td>
                    <td className="px-5 py-3.5 text-gray-400 text-sm">{format(new Date(exp.date), 'MMM d, yyyy')}</td>
                    <td className="px-5 py-3.5 text-right">
                      <span className="text-red-400 font-semibold text-sm">-₹{exp.amount.toLocaleString('en-IN')}</span>
                    </td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => setEditItem({ ...exp, date: format(new Date(exp.date), 'yyyy-MM-dd') })}
                          className="w-7 h-7 rounded-lg bg-accent-violet/10 hover:bg-accent-violet/20 flex items-center justify-center
                                     text-accent-violet transition-all duration-200">
                          <Edit2 size={13} />
                        </button>
                        <button onClick={() => setDeleteId(exp._id)}
                          className="w-7 h-7 rounded-lg bg-red-500/10 hover:bg-red-500/20 flex items-center justify-center
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Add Expense">
        <ExpenseForm onSubmit={handleAdd} onClose={() => setShowModal(false)} loading={formLoading} />
      </Modal>

      {/* Edit Modal */}
      <Modal open={!!editItem} onClose={() => setEditItem(null)} title="Edit Expense">
        <ExpenseForm initial={editItem} onSubmit={handleEdit} onClose={() => setEditItem(null)} loading={formLoading} />
      </Modal>

      {/* Delete Confirm */}
      <ConfirmDialog open={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete}
        loading={deleteLoading} title="Delete Expense"
